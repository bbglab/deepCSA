...
$$
\text{expected mutation density (muts/bp)} = \text{Mutation rate (mutations} \times \text{genome)} \times \text{time (year?)} = \frac{\text{Mutations}}{\text{Genome size (bp)}} \times (\text{time})
$$
