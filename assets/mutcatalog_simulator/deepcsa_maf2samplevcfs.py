...
def build_vcf_like_dataframe(mutations_dataframe, sample):
    """
    ...
    if "INFO" not in mutations_dataframe.columns:
        print(f"WARNING: INFO column is missing from the mutations dataframe. Setting it to 'SAMPLE={sample};'")
        mutations_dataframe["INFO"] = f"SAMPLE={sample};"
...
def main(mutations_file, output_dir, maf_from_deepcsa, sample_name_column):
    ...
    for sample in mutations[sample_name_column].unique():
        ...
        label = '.'.join((os.path.basename(mutations_file)).split('.')[:-1])
        sample_file = f"{output_dir}/{label}.vcf"
