...
workflow DEEPCSA {
    ...
    if (params.vep_species == 'homo_sapiens') {
        DNA2PROTEINMAPPING(
            MUT_PREPROCESSING.out.mutations_all_samples,
    ...
    if (params.profileall) {
        ...
        if (run_mutdensity) {
            ...
            // Concatenate all outputs into a single file
            MUTDENSITYADJUSTED.out.mutdensities
                .map { it -> it[1] }
                .flatten()
            ...
            MUTDENSITYADJUSTED.out.mutdensities_flat
                .map { it -> it[1] }
                .flatten()
    ...
    if (params.oncodrivefml) {
        ...
        if (params.profilenonprot) {
            ...
            ONCODRIVEFMLNONPROT(
                somatic_mutations,
                MUTABILITYNONPROT.out.mutability,
                CREATEPANELS.out.exons_consensus_panel,
                cadd_scores,
                mode,
            )
    ...
    if (params.oncodrive3d) {
        if (params.profileall) {
            ...
            ONCODRIVE3D(
                ...
                datasets3d,
                annotations3d,
                MUT_PREPROCESSING.out.all_raw_vep_annotation,
            )
