process FILTER_CAPTURED_PANEL {
    ...
    tuple val(meta2), path(flagged_bed)

    output:
    tuple val(meta), path("captured_panel.tab.filtered.tsv")     , emit: filtered_annotated_panel
    path("captured_panel.tab.removed_sites.tsv")                 , emit: removed_sites
    ...
    # Create header for removed sites file with additional columns
    head -n 1 ${complete_annotated_panel} | awk '{print \$0 "\\tBED_CHROM\\tBED_START\\tBED_END\\tFILTER"}' > captured_panel.tab.removed_sites.tsv
    ...
    # Create a separate file with removed sites
    bedtools intersect -a temp_captured_panel.bed -b ${flagged_bed} -wa -wb | cut -f4- >> captured_panel.tab.removed_sites.tsv
    ...
    touch captured_panel.tab.removed_sites.tsv
