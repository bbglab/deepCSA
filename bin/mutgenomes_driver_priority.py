...
def recode_csqn_type(df, gene, new_csqn_type):
    ...
    df['canonical_Consequence_broader'] = df.apply(lambda s: \
        new_csqn_type if s['canonical_SYMBOL'] == gene \
        else s['canonical_Consequence_broader'], 
        axis=1)
    return df
    

def snv_am(sample, somatic_mutations_file, omega_file, recoded_genes=[]):
    ...
    for gene in recoded_genes:
        ...
        print(gene, somatic_mutations[somatic_mutations['canonical_SYMBOL'] == gene]['canonical_Consequence_broader'].unique())
...
def snv_nd(sample, somatic_mutations_file, omega_file, recoded_genes=[]):
