...
def flag_repetitive_variants(
    maf_df: pd.DataFrame, repetitive_variant_threshold: int, somatic_vaf_boundary: float
) -> pd.DataFrame:
    """
    ...
    maf_df_f_somatic = maf_df.loc[somatic_mask(maf_df, somatic_vaf_boundary)][
        ["MUT_ID", "SAMPLE_ID", "PMEAN", "PSTD"]
    ...
    repetitive_variants = maf_df_f_somatic_pivot[maf_df_f_somatic_pivot["count"] >= repetitive_variant_threshold][
        "MUT_ID"
    ]
    ...
    maf_df_f_somatic_pos_info = maf_df_f_somatic[
        ~(maf_df_f_somatic["PMEAN"].isna()) & (maf_df_f_somatic["PMEAN"] != -1) & (maf_df_f_somatic["PSTD"] == 0)
    ]
    ...
    maf_df["FILTER"] = maf_df[["FILTER", "repetitive_mapping_variant"]].apply(
        lambda x: add_filter(x["FILTER"], x["repetitive_mapping_variant"], "repetitive_mapping_variant"), axis=1
    )
...
def flag_cohort_n_rich(
    maf_df: pd.DataFrame, n_rich_cohort_proportion: float, somatic_vaf_boundary: float
) -> pd.DataFrame:
    """
    ...
    n_rich_vars_df = (
        ...
        .groupby("MUT_ID")
        .agg(N_rich_frequency=("SAMPLE_ID", "count"), VAF_Ns_threshold=("VAF_Ns", "min"))
    )
    ...
    maf_df["FILTER"] = maf_df[["FILTER", "cohort_n_rich_threshold"]].apply(
        lambda x: add_filter(x["FILTER"], x["cohort_n_rich_threshold"], "cohort_n_rich_threshold"), axis=1
    )
...
def flag_other_samples_snp(maf_df, somatic_vaf_boundary: float) -> pd.DataFrame:
    """
    ...
    germline_vars_all_samples = maf_df.loc[germline_mask(maf_df, somatic_vaf_boundary), "MUT_ID"].unique()
    ...
    maf_df.loc[
        (maf_df["MUT_ID"].isin(germline_vars_all_samples)) & somatic_mask(maf_df, somatic_vaf_boundary),
        "other_sample_SNP",
    ] = True
...
def flag_gnomad_snp(maf_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "gnomAD_SNP" in maf_df.columns:
        maf_df["gnomAD_SNP"] = (
            maf_df["gnomAD_SNP"].replace({"True": True, "False": False, "-": False}).fillna(False).astype(bool)
        )
        ...
        maf_df["FILTER"] = maf_df[["FILTER", "gnomAD_SNP"]].apply(
            lambda x: add_filter(x["FILTER"], x["gnomAD_SNP"], "gnomAD_SNP"), axis=1
        )
...
def flag_vaf_ns_threshold(maf_df: pd.DataFrame, vaf_ns_threshold: float) -> pd.DataFrame:
    ...
    maf_df["FILTER"] = maf_df[["FILTER", "high_n_vaf"]].apply(
        lambda x: add_filter(x["FILTER"], x["high_n_vaf"], "high_n_vaf"), axis=1
    )
...
def flag_distorted_expanded(maf_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "VAF_distorted_expanded_sq" in maf_df.columns:
        maf_df["FILTER"] = maf_df[["FILTER", "VAF_distorted_expanded_sq"]].apply(
            lambda x: add_filter(x["FILTER"], x["VAF_distorted_expanded_sq"], "VAF_distorted_expanded_sq"), axis=1
        )
...
def flag_maf(
    ...
    maf_df.to_csv(f"{sample_name}.cohort.filtered.tsv.gz", sep="\t", header=True, index=False)
...
@click.option(
    "--vaf-ns-threshold", required=False, type=float, default=0.1, help="VAF of Ns threshold for filtering variants"
)
def main(
    maf_df_file: str,
    sample_name: str,
