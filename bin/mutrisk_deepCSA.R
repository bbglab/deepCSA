...
mutation_rates_all_depth = estimate_rates_ie(mle_submodel, genemuts = genemuts, RefCDS_1_genome = RefCDS_1_genome)

# intergrate with chromosome number
mutation_rates_all_depth
gene_chrs = consensus_depths |> 
  select(chr, GENE) |> distinct() |> 
  dplyr::rename(gene_name = GENE)
per_cell = left_join(gene_chrs, mutation_rates_all_depth) |> 
...
write.table(mutation_rates_all_depth, paste0(output_folder, "/mutation_rates_all_depth.tsv"), sep = "\t", quote = FALSE, row.names = FALSE)
...
dnds_by_sample = ggplot(globaldnds_all, aes(x = reorder(sample, mle), y = mle)) +
  ...
  scale_y_continuous(expand=expansion(mult=c(0,0.1))) + 
  labs(x = NULL)
ggsave(paste0(output_folder, "/dnds_by_sample.png"), dnds_by_sample, width = 3 + (length(globaldnds_all)/4), height = 5)
