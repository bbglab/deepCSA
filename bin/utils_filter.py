...
def filter_maf(maf_df, filter_criteria):
    ...
    operators = {
        ...
        "ge": lambda x, y: x >= y,
        "not": lambda x, y: x != y,
        "notcontains": lambda x, y: x.apply(
            lambda z: y not in z.split(";")
        ),  # (~maf_df["FILTER"].str.contains("not_in_panel"))
    ...
    for col, criterion in filter_criteria:
        if isinstance(criterion, bool):
            ...
            print(
                f"Applying {col}:{criterion} filter implied going from {pref_len} mutations to {maf_df.shape[0]} mutations."
            )
...
def somatic_mask(maf_df: pd.DataFrame, threshold: float) -> pd.Series:
    """
    Return a boolean mask identifying somatic variants.

    Parameters
    ----------
    maf_df : pd.DataFrame
        MAF dataframe containing at least the columns ``VAF``, ``vd_VAF``, and
        ``VAF_AM``.
    threshold : float
        Upper bound (inclusive) on VAF for a variant to be called somatic.

    Returns
    -------
    pd.Series
        Boolean series with the same index as *maf_df*; ``True`` where the
        variant is somatic.
    """
    return (maf_df["VAF"] <= threshold) & (maf_df["vd_VAF"] <= threshold) & (maf_df["VAF_AM"] <= threshold)
...
def germline_mask(maf_df: pd.DataFrame, threshold: float) -> pd.Series:
    ...
    maf_df : pd.DataFrame
        ...
        ``VAF_AM``.
    threshold : float
        Lower bound (exclusive) on VAF for a variant to be called germline.
...
def extract_flagged_regions_bed(
    maf_df: pd.DataFrame, name: str, filters: list[str], specification: str = ""
) -> pd.DataFrame | None:
    """
    ...
    _bed_melt = pd.melt(bed_df, id_vars=["CHROM", "POS"], value_vars=filter_columns, var_name="FILTERS").query(
        "value == True"
    )
    ...
    (
        bed_annotated[["CHROM", "START", "END", "FILTERS"]].to_csv(
            f"{name}.{specification}flagged-pos.bed", sep="\t", header=False, index=False
        )
    )
